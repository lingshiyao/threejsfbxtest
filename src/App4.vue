...
const init = async () => {
  ...
  loader.setPath(nowChooseModelPath).load(modelFile, function (object) {
    ...
    if (!move) {
      ...
      // controls.object.position.set(-211.2455739643911, 27.7561178581811, -103.28825900620915)
      // controls.target.set(-1.590955966957181, 49.24630123985267, -1.8165685296161602);
...
const downloadSingleFileWithPose = async (pose) => {
  console.log(pose.length)
  for (let i = 0; i < pose.length; i++) {
    await sleep(500)
    // console.log(i,pose[i].file )
    ...
    if (item != null) {
      ...
      // saveAsWithObject(downloadFile, i + ".json")
    } else {
      const downloadFile = {
...
<template>
  <div class="base">
    ...
    <div class="threeDBase">
      ...
      <div @click="downloadResult()">download result</div>
      <div @click="release()">release</div>
